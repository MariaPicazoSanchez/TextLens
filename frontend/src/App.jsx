...
export default function App() {
  const [theme, setTheme] = useState(() => localStorage.getItem("textlens-theme") ?? "dark");

  useEffect(() => {
    document.documentElement.classList.toggle("light", theme === "light");
    localStorage.setItem("textlens-theme", theme);
  }, [theme]);
  ...
  const [history, setHistory] = useState(() => {
    try {
      const saved = localStorage.getItem("textlens-history");
      if (!saved) return [];
  ...
  // Persist history to localStorage (skip streaming items)
  useEffect(() => {
    const toSave = history.filter((item) => !item.streaming);
    localStorage.setItem("textlens-history", JSON.stringify(toSave));
  }, [history]);
